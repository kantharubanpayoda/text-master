...
var createApiSignature = function(publicKey, privateKey) {
  ...
  return {
    DATE: date,
    SIGNATURE: crypto.createHmac('sha1', privateKey).update(date).digest('hex'),
    APIKEY: publicKey,
  ...
  return {
    ...
    project: {
      ...
      launch: function(params, cb) {
        sendPut('clients/projects/' + getProject(params) + '/launch', cb);
      }
    },
    document: {
      complete: {
      ...
      get: {
        ...
        filter: function(params, data, cb) {
          sendGet('clients/projects/' + getProject(params) + '/documents/filter' + getId(params), data, cb);
        },
      },
      update: function(params, data, cb) {
